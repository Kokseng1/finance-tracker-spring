...
function ExpenseCategoryPage() {
  ...
  const getCategory = () => {
    axiosInstance
      .get(`http://localhost:8080/expense_category/${id}`, {
        withCredentials: true,
      })
      .then((response) => {
        setCategory(response.data);
        setSearchedCategoryName(response.data.name);
      })
      .catch((err) => {
        console.log(err);
      });
  ...
  const handleAddExpense = async (e) => {
    ...
    const categoryAdded = categories.find(
      (cat) => cat.name === searchedCategoryName
    );
    if (!categoryAdded) {
  ...
  const getdata = async () => {
    try {
      const response = await axiosInstance.get(
        `http://localhost:8080/expense_category/${id}/expenses`,
        {
          withCredentials: true,
        }
      );
